...
module Data.SimpleDatabase.Pure (
    module Data.SimpleDatabase.Pure
...
insertDBEntryAuto db@Database{..} dbEntry =
    case dbEntry of
      ...
      DBESI key value -> db { dbsi = HM.insert key value dbsi}
      DBEID key value -> db { dbid = IM.insert key value dbid}
      DBESD key value -> db { dbsd = HM.insert key value dbsd}
      DBEIS key value -> db { dbis = IM.insert key value dbis}
      DBESS key value -> db { dbss = HM.insert key value dbss}
      DBEIIL key value -> db { dbiil = IM.insert key value dbiil}
      DBESIL key value -> db { dbsil = HM.insert key value dbsil}
      DBEIDL key value -> db { dbidl = IM.insert key value dbidl}
      DBESDL key value -> db { dbsdl = HM.insert key value dbsdl}
      DBEISL key value -> db { dbisl = IM.insert key value dbisl}
      DBESSL key value -> db { dbssl = HM.insert key value dbssl}
...
insertDBSSL key value db@Database{..} =
  db { dbssl = HM.insert key value dbssl }
